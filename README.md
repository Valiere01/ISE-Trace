...
| **`openclaw_gen_data`** | **Stage 2 + 3。** 驱动本地 [OpenClaw](https://github.com/openclaw/openclaw) Agent 进行多轮、role-locked 的用户模拟；把每个工具调用放到真实 OS 上、在隔离的 worker workspace 中执行；归档完整 session 并转换成可训练的 *middle format*。 | [github.com/NairongZheng/openclaw_gen_data](https://github.com/NairongZheng/openclaw_gen_data) |
...
- 可训练的 middle format → `output/middle_format/`
...
├── data_examples/            #  示例 intents、session、middle format
└── docs/                     #  架构、运行模式、部署
```
